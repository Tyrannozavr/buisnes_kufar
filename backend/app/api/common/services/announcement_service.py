...
class PublicAnnouncementService:
    ...
    async def get_company_announcements(self, company_id: int, page: int = 1, per_page: int = 10) -> PublicAnnouncementListResponse:
        """Получить опубликованные объявления компании"""
        announcements, total = await self.announcement_repository.get_published_by_company_id(company_id, page, per_page)
    ...
    async def get_announcements_by_category(self, category: str, page: int = 1, per_page: int = 10) -> PublicAnnouncementListResponse:
