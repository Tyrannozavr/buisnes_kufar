...
class CompanyRelationsRepository:
    ...
    async def remove_relation(self, company_id: int, related_company_id: int, relation_type: CompanyRelationType) -> bool:
    ...
    async def get_relations(self, company_id: int, relation_type: Optional[CompanyRelationType] = None) -> List[CompanyRelation]:
    ...
    async def get_related_to(self, related_company_id: int, relation_type: Optional[CompanyRelationType] = None) -> List[CompanyRelation]:
    ...
    async def get_related_companies(self, company_id: int, relation_type: CompanyRelationType, page: int = 1, per_page: int = 10):
        stmt = (
            select(Company)
