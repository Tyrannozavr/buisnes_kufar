...
Репозиторий для владельца компании. Все операции могут производиться только пользователем, на которого указывает
`user_id` компании.
...
Система загрузки изображений продуктов была обновлена для сохранения файлов в директорию вместо хранения base64-encoded
строк в базе данных. Это улучшает производительность и уменьшает размер базы данных.
