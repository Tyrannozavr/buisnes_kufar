...
class CompanyProductsRepository:
    ...
    async def get_by_company_id_and_slug(
        self, company_id: int, slug: str
    ) -> Optional[Product]:
        """Продукт продавца по slug (активный, не скрытый). Для checkout при несовпадении article."""
        query = select(Product).options(selectinload(Product.company)).where(
            and_(
