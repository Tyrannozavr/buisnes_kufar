#!/usr/bin/env python3
"""
Простой скрипт для заполнения единиц измерения
"""
...
async def fill_units_of_measurement():
    ...
    async with async_session() as session:
        try:
            ...
            # Добавляем единицы измерения через SQL
            for unit_data in units_data:
                await session.execute(text("""
                    INSERT INTO units_of_measurement (name, symbol, code, created_at, updated_at)
                    VALUES (:name, :symbol, :code, NOW(), NOW())
                """), {
                    "name": unit_data["name"],
