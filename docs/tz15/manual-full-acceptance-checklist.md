...
**Статус:** ✅ **ТЗ_15 реализовано и выкатано** (17.07.2026, ветка `preprod` → https://tradesynergy.ru)

**Dev UI:** http://localhost/ · **Prod:** https://tradesynergy.ru  
**Пароль:** `123456`
...
Снято 17.07.2026 — ТЗ реализуется целиком:
- Поиск транспорта — рабочий список перевозчиков/экспедиторов
- Транспорт / Водители — CRUD
- Договор экспедиции / УПД — создание и отображение в таблице
- Одна таблица «Товары» (вкладка «Услуги» убрана)

Остаётся вне юр. ЭДО: полноценный XML/ЭТрН-бланк УПД как в Диадок — визуальный документ в редакторе.
...
- Бланк УПД / договора экспедиции в редакторе может быть упрощённым (запись + переход есть; полный ЭДО/XML — вне ТЗ).
- **Dependabot:** на GitHub ~63 уязвимости в зависимостях.
...
|-----|-----------|
| `preprod` на сервере | ✅ |
| `alembic upgrade head` → `f9b0c1d2e3f4` | ✅ |
| Seed test users | ✅ |
| `tz15_api_smoke.sh` на tradesynergy.ru | ✅ 14/14 |
