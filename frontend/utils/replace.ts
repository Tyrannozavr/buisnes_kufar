 ...
 * @example "{{ СРОК_ОПЛАТЫ_СЧЕТА_ДОГОВОРА }}" -> "10"
 * @example "{{ СРОК_ОПЛАТЫ_СЧЕТА_ОФЕРТЫ }}" -> "10"
...
export const replaceFields = (line: string, billData: BillData): string => {
	return line
		...
		.replaceAll(
			"{{ СРОК_ПОСТАВКИ_СЧЕТА_ДОГОВОРА }}",
			billData.deliveryTermsContract
		)
		.replaceAll(
			"{{ НАЗВАНИЕ_КОМПАНИИ_ПОСТАВЩИКА }}",
			billData.seller.companyName ?? ""
		)
}
